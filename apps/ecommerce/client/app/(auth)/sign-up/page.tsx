...
const Signup = () => {
  ...
  if (!mrkFeatures.customerAuthEnabled) {
    return (
      <MainLayout>
        <div className="flex min-h-[70vh] items-center justify-center p-4 sm:p-6">
          <main className="w-full max-w-md rounded-sm border border-gray-200 bg-white p-6 text-center shadow-sm sm:p-8">
            <h1 className="text-2xl font-semibold text-gray-900">
              Customer registration is disabled
            </h1>
            <p className="mt-3 text-sm leading-6 text-gray-600">
              MRK currently uses catalog, enquiry, phone, WhatsApp, and dealer
              application flows instead of public customer accounts.
            </p>
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Link
                href="/products"
                className="rounded-sm bg-indigo-600 px-4 py-3 text-sm font-semibold text-white hover:bg-indigo-700"
              >
                View Catalog
              </Link>
              <Link
                href="/contact"
                className="rounded-sm border border-gray-300 px-4 py-3 text-sm font-semibold text-gray-800 hover:border-indigo-500 hover:text-indigo-700"
              >
                Contact MRK
              </Link>
            </div>
            <Link
              href="/sign-in"
              className="mt-5 inline-block text-sm font-semibold text-indigo-700 hover:text-indigo-800"
            >
              Admin sign in
            </Link>
          </main>

...
import { DEMO_ACCOUNT_EMAILS, isDemoMode } from "@/app/lib/demo";
...
const SignIn = () => {
  ...
  const errorMessage =
    ...
    (error as any)?.status === "TIMEOUT_ERROR"
      ? "API server is not running. Start it with npm.cmd run api:dev."
      : (error as any)?.data?.message || "Unable to sign in. Check your credentials.";
        ...
        <main className="w-full max-w-md bg-white rounded-lg shadow-lg p-6 sm:p-8">
          ...
          {mrkFeatures.customerAuthEnabled ? (
            ...
            <p className="mt-4 text-center text-xs text-gray-500">
              Public customer accounts are disabled. Use this page for admin
              access.
            </p>
          )}

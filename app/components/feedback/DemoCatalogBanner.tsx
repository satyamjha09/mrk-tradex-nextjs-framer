  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl flex-col items-start gap-3 px-4 py-3 sm:flex-row sm:items-center sm:px-6 lg:px-8">
        ...
        <div className="w-full min-w-0 flex-1 text-sm leading-snug text-amber-950 sm:w-auto">
          <p>
            <span className="font-semibold">Static sample catalog</span>
            {" - "}
            MRK catalog listings are placeholder technical records, not final
            live product data.
            {forced ? (
              <>
                {" "}
                Catalog preview is turned on in this build; admin and sign-in
                can still use the live API when it is running.
              </>
            ) : (
              <>
                {" "}
          ...
          <p className="mt-1 text-amber-900/85">
            Browse-only preview. Public cart, checkout, payments, customer
            orders, and reviews stay disabled for the MRK enquiry model.{" "}
            <Link
